...
/**
 */
void reader_z_uv_cm(char* fname, int fid, obsmeta* meta, grid* g, observations* obs)
{
//    int ksurf = grid_getsurflayerid(g);
    ...
    size_t dimlen_z[NC_MAX_VAR_DIMS], dimlen_var[NC_MAX_VAR_DIMS];
    ...
    double *z = NULL;
    float z_add_offset, z_scale_factor, z_fill_value;
    ...
    if (varname == NULL)
        enkf_quit("reader_z_uv_cm(): %s: VARNAME not specified", fname);
    else
    ...
    if (rotation_flag != NULL) {
        ...
        if (valid_rotation_flag) {
            ...
            if (need_rotation == 1 && grid_angle == NULL)
                enkf_quit("reader_z_uv_cm(): %s cannot rotate vectors without ANGLE parameter defined within grid prm file",fname);
            else
                enkf_printf("\t#Rotation of vectors enabled\n");
        }
        else
            enkf_quit("reader_z_uv_cm(): %s invalid ROTATION parameter. Need to be a boolean digit");
    }
...
//    int ndim_in_h = 0;
    ...
    if (lonname != NULL) {
        ...
        enkf_quit("reader_z_uv_cm(): %s: could not find longitude variable\n",
                fname);
    ...
    if (latname != NULL) {
        ...
        enkf_quit("reader_z_uv_cm(): %s: could not find latitude variable\n",
                fname);
    ...
    if (nobs == 1)
        enkf_quit("reader_z_uv_cm(): %s: No observations\n");
    ...
    if (gcoord_mismatch)
        enkf_quit("reader_z_uv_cm(): %s: dimension number mismatch between lon/lat",
                fname);
    ...
    if (gcoord_is_singleton) {
        ...
        enkf_quit("reader_z_uv_cm(): %s: Longitude has %s and latitude "
    ...
    if (!missing_depth) {
        ...
        if (ncw_att_exists(ncid, varid_z, "add_offset")) {
            ncw_get_att_float(ncid, varid_z, "add_offset", &z_add_offset);
            ncw_get_att_float(ncid, varid_z, "scale_factor", &z_scale_factor);
            for (i = 0; i< dimlen_z[0]; ++i)
                z[i] = z[i]*z_scale_factor + z_add_offset;
        }
        if (ncw_att_exists(ncid, varid_var, "_FillValue"))
            ncw_get_att_float(ncid, varid_z, "_FillValue", &z_fill_value);
        nz = dimlen_z[0];
    } else {
        enkf_printf("        WARNING: assuming first dimension in %s is along Z.\n",varname);
        if (dimlen_var[0] != nobs)
             enkf_quit("reader_z_uv_cm(): %s: NOBS mismatch.",fname);

        z = malloc(dimlen_var[0]*sizeof(double));
        for (i = 0; i < dimlen_var[0]; ++i)
            z[i] = abs(instrument_depth);
        nz = dimlen_var[0];
    }
    ...
    if (timename != NULL) {
        ...
        enkf_quit("reader_z_uv_cm(): %s: no TIME variable\n", fname);
    }
    ...
    if (need_rotation == 1) {
        ...
        if (u_varname != NULL)
            ...
            enkf_quit("reader_z_uv_cm(): %s: Variable %s not found.",fname, u_varname);
        ...
        if (v_varname != NULL)
            ...
            enkf_quit("reader_z_uv_cm(): %s: Variable %s not found.",fname, v_varname);
    ...
    for (i = 0; i < (int) nobs; ++i) {
        ...
        invalid_z_var = (z != NULL && (z[i] == z_fill_value || isnan(z[i])));

        if (invalid_var || invalid_std || invalid_estd || invalid_time || invalid_u_var || invalid_v_var || invalid_z_var)
            continue;
