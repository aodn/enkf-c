 ...
 *              - U_STD ("UCUR_sd") (-)
 *                  Standard deviation of zonal velocity. same as above.
 *              - V_STD ("VCUR_sd") (-)
 *                  Standard deviation of meridonal velocity. same as above.
 *              - U_VARSHIFT (-)
...
void reader_xy_gridded_imos_hfradar(char* fname, int fid, obsmeta* meta, grid* g, observations* obs)
{
    ...
    /* UV requirements */
    int varid_u = -1, varid_v =-1, varid_u_std = -1, varid_v_std = -1;
    float* u_var = NULL;
    float* v_var = NULL;
    float* u_std = NULL;
    float* v_std = NULL;
        ...
        if (estd == NULL && std == NULL){
            ...
            if (std == NULL)
                ...
                if (!isnan(std_add_offset))
                    /* TODO adjust std in the rotated case */
