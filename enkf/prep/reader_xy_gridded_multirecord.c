/******************************************************************************
 *
 * File:        reader_xy_gridded_multirecord.c        
 *
 * Created:     08/03/2017
 *
 * Author:      Pavel Sakov
...
void reader_xy_gridded_multirecord(char* fname, int fid, obsmeta* meta, grid* g, observations* obs)
{
    ...
    int iscurv = -1;
    size_t ni = 0, nj = 0, nt = 0, n = 0, n_var = 0;
    ...
    if (varname == NULL)
        enkf_quit("reader_xy_gridded_multirecord(): %s: VARNAME not specified", fname);
    else
    ...
    if (ndim_var == 3) {
        ...
        enkf_quit("reader_xy_gridded_multirecord(): %s: %s: %d dimensions (must be 2 or 3)", fname, varname, ndim_var);
    ...
    if (lonname != NULL) {
        ...
        enkf_quit("reader_xy_gridded_multirecord(): %s: could not find longitude variable", fname);
    ...
    if (ndim_xy == 1) {
        ...
        enkf_quit("reader_xy_gridded_multirecord(): %s: coordinate variable \"%s\" has neither 1 or 2 dimensions", fname, lonname);
    ...
    if (dimlen_var[ndim_var - 1] != ni)
        enkf_quit("reader_xy_gridded_multirecord(): %s: %s: longitude must be the inner coordinate", fname, varname);
    ...
    if (timename != NULL) {
        ...
        enkf_printf("        reader_xy_gridded_multirecord(): %s: no TIME variable\n", fname);
        have_time = 0;
    }
